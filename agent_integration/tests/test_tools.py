...
def test_lookup_schedule_known_course():
    out = lookup_schedule("CSE 142", "Fall 2026")
    assert "CSE 142" in out
    assert "Fall 2026" in out
    assert "16:00" in out and "MWF" in out
    assert "sample" in out.lower()   # schedule/seat data is labeled as sample, not live


def test_lookup_schedule_code_normalization():
    # 'cse142' should normalize to 'CSE 142'
    assert "CSE 142" in lookup_schedule("cse142", "Fall 2026")


def test_lookup_schedule_not_found():
    out = lookup_schedule("CSE 999")
    assert "no offering" in out.lower()


def test_calendar_known_quarter():
    out = get_academic_calendar("Winter 2027")
    assert "January 4" in out          # instruction begins
    assert "2027-01-04" in out         # ISO form also present


def test_calendar_lists_all_when_no_term():
    out = get_academic_calendar()
    assert "Fall 2026" in out and "Winter 2027" in out and "Spring 2027" in out
